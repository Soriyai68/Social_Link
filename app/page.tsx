...
const socials = [
  ...
  { name: 'Facebook', mark: 'f', handle: 'ហាងលក់ម៉ូតូ RPRU', description: 'ហាងលក់ម៉ូតូ RPRU.', className: 'bg-[#1877f2] text-white', href: 'https://www.facebook.com/profile.php?id=61571429474635' },
      ...
      <div className="relative mx-auto flex min-h-screen max-w-6xl flex-col px-5 py-7 sm:px-8 lg:px-12">
        <header className="flex items-center justify-between">
          <a href="#top" className="flex items-center gap-3" aria-label="Go to top">
            <span className="grid size-10 place-items-center rounded-2xl bg-slate-950 text-sm font-bold text-white shadow-lg shadow-slate-900/10">YN</span>
        ...
        <section id="top" className="mx-auto flex w-full max-w-3xl flex-col items-center pb-14 pt-20 text-center sm:pt-28">
          ...
          <h1 className="max-w-2xl text-balance text-5xl font-semibold leading-[0.98] tracking-[-0.06em] text-slate-950 sm:text-7xl">
            Find me <span className="bg-gradient-to-r from-emerald-500 to-sky-500 bg-clip-text text-transparent">everywhere.</span>
          </h1>
          <p className="mt-7 max-w-xl text-pretty text-base leading-7 text-slate-600 sm:text-lg">
            A little corner of the internet for my latest ideas, projects, and everyday moments. Follow along wherever you feel at home.
          </p>
          <div className="mt-8 flex items-center gap-3 text-sm text-slate-500">
            ...
            <span>One person, many places.</span>
          </div>
        </section>
        ...
        <footer className="mt-auto flex flex-col gap-3 border-t border-slate-200/80 py-6 text-sm text-slate-500 sm:flex-row sm:items-center sm:justify-between">
          ...
          <span className="flex items-center gap-2"><Play className="size-3 fill-emerald-500 text-emerald-500" /> Built to share good energy.</span>
        </footer>
      </div>
    </main>
  )
